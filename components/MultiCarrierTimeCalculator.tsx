...
export function MultiCarrierTimeCalculator() {
  const [quotesPerWeek, setQuotesPerWeek] = useState(30);
  const [minPerQuote, setMinPerQuote] = useState(25);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
          <span style={{ fontSize: '0.8125rem', color: '#cbd5e1' }}>
            Estimated min / quote (manual, per carrier)
          </span>
          <input
            type="number"
            min={1}
      ...
      >
        ...
        <p style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.375rem' }}>
          One workflow across all carriers—no rekeying.
        </p>
      </div>
    </div>
  );
